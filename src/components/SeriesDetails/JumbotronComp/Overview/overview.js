...
export default function Overview({ overview = "", director, genres, languages }) {
          ...
          <Container>
            ...
            {genres && (
              <Row style={CardStyle.Details()}>
                ...
                <Col md={11} className="jumbotron-overview-card-type">
                  {genres.map(
                    (genres, index) =>
                      genres && <span key={index} className="gradient-text">{genres.title} &nbsp;</span>
                  )}
                </Col>
              </Row>
            )}
            {languages && (
              <Row style={CardStyle.Details()}>
                ...
                {languages.map(
                    (language, index) =>
                    language && <span key={index} className="gradient-text">{language.title} &nbsp;</span>
                  )}
                </Col>
              </Row>
            )}
